/*
**  File Description: PQDIF Physical Format Definitions
**
**  This file contains the complete definitions for the physical format of a PQDIF file.
**  It contains no information about the logical structure, which is defined in PQDIF_LG.H
**
** --------------------------------------------------------------------------
**
...
//  The structures shall be 1-byte packed.
#ifdef __BORLANDC__
#pragma pack( 1 )
#elif _MSC_VER
#pragma pack( push, 1 )
...
**                      (i.e., the last character shall be a NULL, to
...
**  Excel counts days since December 30, 1899, converts to a double
**  and adds to this a number less than 1.0 that is the fractional day. This structure enhances
**  accuracy of this method by separating out the days since 1900 and seconds since midnight.)
**
**  To eliminate that time stamps can be interpreted with no ambiguity all
**  time stamps are referenced to Coordinated Universal Time (UTC).
*/
typedef struct ts
{
    UINT4   day;        //  days since December 30, 1899 UTC
    //  0xFFFFFFFF -> 4294967295 days.
...
**
**  The Unique Universal Identifier (UUID) is also known as a
**  Globally Unique Identifier (GUID). For practical purposes,
**  a newly generated GUID is unique, without depending
**  on a central registration authority or coordination
**  between the parties generating them, While the probability
**  that a GUID will be duplicated is not zero, it is close enough
**  to zero to be negligible.

**  This allows PQDIF implementors to generate "private" tags
**  that should not conflict with the standard tags.
**
**  GUID tags are used to mark the header of each record,
**  as well as to identify the elements in the internal structures.
*/
#ifndef GUID_DEFINED
#define GUID_DEFINED
...
**  12/30/1899 and 1/1/1970. This is used to convert between
**  "C Time" and Excel style day counts.
**  "C Time" is a 4 byte integer representing the number of seconds
**  elapsed since  January 1, 1970.
**  Excel time is an 8 byte real number that represents the number of days
**  elapsed since 12/30/1899.
**  The fractional part is therefore convertible to seconds since midnight.
*/
...
**
**  ======================================================================
**  Fundamental premise for file format writing: File shall be capable of
...
**  ======================================================================
**  The record body shall begin with a Collection element. This is
**  defined below in the record structure.
**
**  ======================================================================
**  The fundamental premise for the standard structures is to
**  create 4 byte alignment. All of the structures conform to this
**  premise. In addition, the data values associated with scalars and
**  vectors shall be padded out to 4-byte multiples. This _total_ size
**  should be specified in the c_collection_element structure (sizeElement).
**
**  ======================================================================
...
**
**  This collection should be created by combining a c_collection with an array
**  of c_collection_element, which provide pointers to the
...
struct c_collection_element
{
    ...
    //  The following 8 bytes may point to another location in the record
    //  or it may hold the actual data value (if it fits in 8 bytes).
